...
const httpsOptions = {
  ...
  cert: fs.readFileSync(
    "/etc/letsencrypt/live/ballershuffle.com/fullchain.pem"
  ),
};
  ...
  try {
    ...
    if (existingUser.length > 0) {
      ...
      if (user.is_verified === 0) {
        ...
        await promiseQuery("DELETE FROM user_push_tokens WHERE user_id = ?", [
          user.id,
        ]);
        // Delete the user row
        await promiseQuery("DELETE FROM users WHERE username = ?", [username]);
        console.log(
          `Deleted unverified user and related push tokens for username: ${username}`
        );
      } else {
  ...
  try {
    ...
    if (!user.is_verified) {
      ...
      const codeExpired =
        ...
        new Date(user.verification_code_expires) < now;
...
app.post("/api/reset-password", async (req, res) => {
  const { code, userId, email, newPassword, pushToken } = req.body;
...
async function sendGameRegistrationNotification(
  userId,
  courtName,
  gameStartTime,
  gameId,
...
app.post(
  "/api/handle-notification-response",
  authenticateToken,
  async (req, res) => {
...
app.post(
  "/api/confirm-game-participation",
  authenticateToken,
  async (req, res) => {
    try {
      const { gameId, playerId, action } = req.body;
...
app.post("/api/bug-report", authenticateToken, async (req, res) => {
  try {
    ...
    await sendBugReportEmail(username, message);
...
app.put(
  "/api/courts/:courtId/toggle-ratings",
  authenticateToken,
  async (req, res) => {
    try {
  ...
  async (req, res) => {
    ...
    p.courtId = ?;
      ...
      if (results.length === 0 || results[0].avgScoring === null) {
        return res
          .status(404)
          .json({ message: "No players found for this court" });
      }
...
app.get(
  "/api/player-full-data/:playerId/:courtId",
  authenticateToken,
  async (req, res) => {
    try {
      ...
      const { results: playerData } = await promiseQuery(playerQuery, [
        playerId,
        courtId,
      ]);
      ...
      const statsQuery =
        ...
        bcs.total_games_played,
          bcs.total_points as total_points,
          bcs.total_wins as wins,
          p.num_of_mvps as mvps,
          ROUND(bcs.total_points / NULLIF(bcs.total_games_played, 0), 1) as ppg,
...
app.get(
  "/api/football_court_averages/:court_id/",
  authenticateToken,
  async (req, res) => {
    try {
      const courtId = req.params.court_id;
    ...
    LEFT JOIN
      ...
      if (results.length === 0 || results[0].avgFinishing === null) {
        return res.status(404).json({
          message: "No players in this court, no averages to present yet.",
        });
      }
...
app.post(
  "/api/create_player/:court_id/:creator_user_fk",
  authenticateToken,
  async (req, res) => {
...
app.post(
  "/api/create_player_football/:court_id/:creator_user_fk",
  authenticateToken,
  async (req, res) => {
    const court_Id = req.params.court_id;
    const creator_user_fk = req.params.creator_user_fk;
...
app.get(
  "/api/football-player/:player_id/:court_id",
  authenticateToken,
  async (req, res) => {
    try {
      const courtId = req.params.court_id;
...
app.put(
  "/api/update-user-profile/:userId/:courtId",
  authenticateToken,
  async (req, res) => {
...
app.get(
  "/api/my-player/:user_id/:court_id",
  authenticateToken,
  async (req, res) => {
    try {
      const courtId = req.params.court_id;
      ...
      if (results.length === 0) {
        return res
          .status(404)
          .json({ message: "No player found for this user in this court" });
      }
...
app.get(
  "/api/is_player_assinged/:player_id",
  authenticateToken,
  async (req, res) => {
    try {
      const playerId = req.params.player_id;
...
// Update Player endpoint
app.put(
  "/api/update_player/:player_id/:court_id",
  authenticateToken,
  async (req, res) => {
    try {
      const courtId = req.params.court_id;
      ...
      await promiseQuery(updateNamePriorityBuild, [
        name,
        priority,
        playerBuild,
        playerId,
...
app.put(
  "/api/update-player-football/:player_id/:court_id",
  authenticateToken,
  async (req, res) => {
    try {
      const courtId = req.params.court_id;
...
app.put(
  "/api/update-player-picture/:player_id/:court_id",
  authenticateToken,
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      const query =
        "SELECT profile_image FROM ballershuffledb.players WHERE id = ?";
...
app.put(
  "/api/assign_player/:player_id/:email/:court_id",
  authenticateToken,
  async (req, res) => {
    try {
      const playerId = req.params.player_id;
      ...
      if (playerCheckResults && playerCheckResults.length > 0) {
        return res.status(400).json({
          message:
            "User is already assigned to a player in the specified court",
        });
      }
...
app.delete(
  "/api/delete_player/:player_id/:court_id",
  authenticateToken,
  async (req, res) => {
    try {
      const courtId = req.params.court_id;
...
app.delete(
  "/api/leave_court/:user_id/:court_id",
  authenticateToken,
  async (req, res) => {
    try {
      const courtId = req.params.court_id;
...
app.get("/api/is_admin/:user_id/:court_id", (req, res) => {
  ...
  const userId = req.params.user_id;
  const query =
    "SELECT is_admin FROM ballershuffledb.court_admins WHERE user_id = ? AND court_id = ?";
...
app.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (results.length === 0) {
        return res.status(404).json({
          message: "No scheduled games found for the specified court ID.",
        });
      }
      ...
      res
        .status(500)
        .json({ message: "An error occurred while fetching scheduled games." });
    }
  }
);
...
app.put("/api/update_game/:gameId", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const hasGameDetailsChanged =
      ...
      currentGame.registration_open_time.getTime() !==
        newRegistrationOpenTime.getTime() ||
      currentGame.registration_close_time.getTime() !==
...
app.get("/api/game/:game_id", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while fetching game details." });
  }
});
...
app.post("/api/register-players", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .send(`${playersIds.length} players registered successfully`);
  } catch (error) {
...
app.get(
  "/api/game_registrations/:game_id",
  authenticateToken,
  async (req, res) => {
    const gameId = req.params.game_id;
    ...
    try {
      ...
      if (results.length === 0) {
        return res
          .status(404)
          .json({ message: "No registrations found for this game" });
      }
...
app.get(
  "/api/can-register-players-to-game/:game_id/:user_id",
  authenticateToken,
  async (req, res) => {
    const gameId = req.params.game_id;
    const userId = req.params.user_id;
...
app.delete("/api/delete_game/:game_id", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    await promiseQuery("DELETE FROM player_game_statistics WHERE game_id = ?", [
      gameId,
    ]);
    ...
    await promiseQuery("DELETE FROM registrations_to_game WHERE game_id = ?", [
      gameId,
    ]);
...
app.get(
  "/api/court-statistics/:courtId/:courtType",
  authenticateToken,
  async (req, res) => {
    try {
      const courtId = req.params.courtId;
      ...
      if (results.length === 0) {
        return res
          .status(404)
          .json({ message: "No statistics found for this court" });
      }
...
//-------Fetch game stats from server for the tale in-game table stats--------

app.get(
  "/api/gameday-stats/:courtType/:gameId",
  authenticateToken,
  async (req, res) => {
    const { courtType, gameId } = req.params;
    const table =
      courtType === "Basketball"
        ? "basketball_gameday_stats"
        : "football_gameday_stats";

    try {
      const { results } = await promiseQuery(
        `SELECT * FROM ${table} WHERE gameday_id = ?`,
        [gameId]
      );
      res.json(results);
    } catch (error) {
...
app.put(
  "/api/update-player-stats/:gameId/:playerId",
  authenticateToken,
  async (req, res) => {
    const { gameId, playerId } = req.params;
    const { stats, courtType } = req.body;

    if (!["Basketball", "Football"].includes(courtType)) {
    ...
    try {
      const updateQuery = `
      INSERT INTO ${table} (gameday_id, player_id, ${Object.keys(stats).join(
        ", "
      )})
    ...
    `;
      ...
      await promiseQuery(updateQuery, [gameId, playerId, ...values, ...values]);
...
app.post(
  "/api/update-court-statistics/:gameId/:courtType",
  authenticateToken,
  async (req, res) => {
    const gameId = req.params.gameId;
    const courtType = req.params.courtType;
    ...
    try {
      // Step 1: Get max_players from the game
      const { results: gameResults } = await promiseQuery(
        "SELECT max_players FROM games WHERE game_id = ?",
        [gameId]
      );
      ...
      // Step 2: Get main players ordered by priority and registration time
      const { results: playerResults } = await promiseQuery(
        `SELECT rtg.player_id
   FROM registrations_to_game rtg
   LEFT JOIN players p ON p.id = rtg.player_id
   ...
   LIMIT ?`,
      ...
      // Step 3: Insert rows with zeroes into gameday stats if not exists
      const gameStatsTable =
        courtType === "Basketball"
          ? "basketball_gameday_stats"
          : "football_gameday_stats";

      for (const { player_id } of playerResults) {
        const { results: existing } = await promiseQuery(
          `SELECT 1 FROM ${gameStatsTable} WHERE gameday_id = ? AND player_id = ? LIMIT 1`,
          [gameId, player_id]
        );

        if (!existing || existing.length === 0) {
          await promiseQuery(
            `INSERT INTO ${gameStatsTable} (gameday_id, player_id) VALUES (?, ?)`,
            [gameId, player_id]
          );
        }
      }
      ...
      if (courtType === "Basketball") {
        await promiseQuery(
          `INSERT INTO basketball_court_statistics(player_id, total_games_played, total_points, total_3pts, total_assists, total_steals, total_blocks, total_wins)
        SELECT
        bgs.player_id,
          1 AS total_games_played,
            IFNULL(SUM(bgs.total_points_today), 0) AS total_points,
      ...
      await promiseQuery(
        `UPDATE games SET stats_approved = TRUE WHERE game_id = ?`,
        [gameId]
      );
...
// PATCH /api/patch-game-statistics/:gameId/:courtType
app.patch(
  "/api/patch-game-statistics/:gameId/:courtType",
  authenticateToken,
  async (req, res) => {
    const { gameId, courtType } = req.params;
    const updates = req.body.updates;

    if (!Array.isArray(updates)) {
      return res
        .status(400)
        .json({ error: "Missing or invalid updates array" });
    }

    try {
      const tableName = courtType === "Basketball" ? "basketball" : "football";
      const statsTable = `${tableName}_court_statistics`;

      for (const update of updates) {
        const { playerId, delta } = update;

        if (!playerId || typeof delta !== "object") continue;
        ...
        for (const [field, value] of Object.entries(delta)) {
          ...
          updateFields.push(`${field} = GREATEST(0, ${field} + ?)`);
          updateValues.push(value);
        }

        if (updateFields.length > 0) {
          await promiseQuery(
            `UPDATE ${statsTable} SET ${updateFields.join(
              ", "
            )} WHERE player_id = ?`,
      ...
      res.status(500).json({ error: "Failed to patch statistics" });
    }
  }
);
...
app.get(
  "/api/player-statistics/:playerId/:courtId",
  authenticateToken,
  async (req, res) => {
    try {
      const playerId = req.params.playerId;
      ...
      const { results: courtResults } = await promiseQuery(courtTypeQuery, [
        courtId,
      ]);
...
app.post(
  "/api/game-players-that-played/:gameId",
  authenticateToken,
  async (req, res) => {
    try {
      const gameId = req.params.gameId;
      ...
      res.json({
        message: "Players added successfully to game_players_played",
      });
    } catch (error) {
      console.error("Error creating game_players_played records:", error);
      res
        .status(500)
        .json({ message: "Error creating game players played records" });
    }
  }
);
...
app.get(
  "/api/player-games/:playerId/:courtId",
  authenticateToken,
  async (req, res) => {
    try {
      const playerId = req.params.playerId;
...
app.post(
  "/api/create_match/:gameday_id/:created_by",
  authenticateToken,
  async (req, res) => {
    ...
    try {
      ...
      const team1Values = team1_players.map((playerId) => [
        match_id,
        playerId,
        1,
      ]);
      ...
      const team2Values = team2_players.map((playerId) => [
        match_id,
        playerId,
        2,
      ]);
...
app.get(
  "/api/gameday_matches/:gameday_id",
  authenticateToken,
  async (req, res) => {
    const gameday_id = req.params.gameday_id;
...
app.get(
  "/api/basketball_gameday_stats/:gameId",
  authenticateToken,
  async (req, res) => {
    const gameId = req.params.gameId;
    ...
    try {
      ...
      WHERE bgs.gameday_id = ?
          ORDER BY(bgs.total_points_today) DESC`,
        [gameId]
      );
      ...
      const statsWithCalculations = results.map((stat) => ({
        ...stat,
        total_points: stat.total_points_today || 0,
      }));
...
app.get(
  "/api/football_gameday_stats/:gameId",
  authenticateToken,
  async (req, res) => {
    const gameId = req.params.gameId;
    ...
    try {
      ...
      const statsWithCalculations = results.map((stat) => ({
        ...stat,
        efficiency:
          stat.total_goals_today /
          (stat.total_goals_today + stat.total_misses_today || 1),
      }));
...
app.get(
  "/api/last-game-details/:court_id",
  authenticateToken,
  async (req, res) => {
    const courtId = req.params.court_id;
...
app.post("/api/notify-teams-ready", authenticateToken, async (req, res) => {
  ...
  try {
    const { results: gameInfo } = await promiseQuery(
      `
      SELECT g.game_id, g.court_id, g.game_start_time, g.max_players, c.courtName
      FROM games g
      JOIN courts c ON g.court_id = c.id
      WHERE g.game_id = ?
      `,
    ...
    // 👇 Get main players by priority + registration time (LIMIT by max_players)
    const { results: mainPlayers } = await promiseQuery(
      `
      SELECT p.user_fk 
      FROM registrations_to_game rtg
      JOIN players p ON p.id = rtg.player_id
      WHERE rtg.game_id = ?
      ORDER BY p.priority ASC, rtg.registration_time ASC
      LIMIT ?
      `,
      [gameId, game.max_players]
    );
    ...
    for (const player of mainPlayers) {
      if (player.user_fk) {
        await sendPushNotification(
          player.user_fk,
          `Teams Ready - ${game.courtName}`,
          `The teams for today's game are ready!\nCheck who you're playing with.`,
          {
            type: "OPEN_GAME",
            courtId: game.court_id,
            gameId: game.game_id,
    ...
    res.status(200).json({ message: "Main players notified about teams" });
  } catch (error) {
    console.error("Error notifying main players about teams:", error);
    res.status(500).json({ message: "Internal server error" });
  }
});
