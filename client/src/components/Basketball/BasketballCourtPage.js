  ...
  return (
    <div className="basketball-home-page-style">
      ...
      <div className="player-list22">
        {players.map((player) => (
          <Link to={`/player/${player.playerId}/${courtId}?courtName=${currCourtName}&courtType=${currCourtType}&userId=${currUserId}`} className="player-link2">
            <div key={player.playerId} className="player-cube2" onMouseEnter={() => setSelectedPlayer(player)} onMouseLeave={() => setSelectedPlayer(null)}>
              {player.name}
              <p>{player.overall}</p>
