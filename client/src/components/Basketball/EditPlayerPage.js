    ...
    if (!nameError && !heightError && !Object.values(attributesErrors).some(error => error !== '')) {
      ...
      if (courtIndex !== -1) {
        ...
        if (playerIndex !== -1) {
         ...
         navigate(`/edit_success/${courtId}?overall=${courts[courtIndex].players[playerIndex].overall}&name=${encodeURIComponent(courts[courtIndex].players[playerIndex].name)}&courtName=${encodeURIComponent(currCourtName)}&courtType=${encodeURIComponent(currCourtType)}&userId=${(currUserId)}`);
       } else {
